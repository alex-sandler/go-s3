...
func (c *Controller) UploadBatch(ctx *fiber.Ctx) error {
	...
	err = c.s3Service.UploadBatch(ctx.Context(), fileData, "image/jpeg")
	if err != nil {
		return ctx.Status(500).JSON(fiber.Map{"error": "Ошибка загрузки файлов в S3"})
	}

	return ctx.JSON(fiber.Map{"message": "Файлы загружены без сжатия"})
}
